...
import de.saxsys.mvvmfx.internal.viewloader.fxml.BeanAdapter;
...
import static de.saxsys.mvvmfx.internal.viewloader.fxml.expression.Operator.*;
...
public abstract class Expression<T> {
    ...
    private static class Parser {
        ...
        public Expression parse(Reader reader) throws IOException {
            ...
            for (Token token : tokens) {
                ...
                case FUNCTION_ARGUMENT: {
                    expression = new ArgumentExpression((String)token.value);
                    break;
                }
                ...
                if (c != -1) {
                    ...
                    if (c == 'n') {
                        if (readKeyword(reader, NULL_KEYWORD)) {
                            ...
                            token = readStaticFunctionExpression(reader, stack);
                            c = reader.read();
                        }
                    } else if (c == '"' || c == '\'') {
        ...
        private Token readStaticFunctionExpression(PushbackReader reader, LinkedList<Token> stack) throws IOException {
            KeyPath path = KeyPath.parse(reader);
            c = reader.read();
            if (c == '(') {
                ...
                for (String token : tokens) {
                    stack.push(new Token(TokenType.FUNCTION_ARGUMENT, token.trim()));
                }

                return new Token(TokenType.STATIC_FUNCTION, path);
            } else {
                return new Token(TokenType.VARIABLE, path);
            }
        }
     ...
     */
    public abstract T evaluate(Object namespace, boolean throwIfNotResolved);
     ...
     */
    public static <T> T get(Object namespace, KeyPath keyPath, boolean throwIfNotResolved) {
        if (keyPath == null) {
            throw new NullPointerException();
        }

        boolean[] keyResolved = throwIfNotResolved ? new boolean[1] : null;
        T result = get(namespace, keyPath.iterator(), keyResolved);
        if (throwIfNotResolved && !keyResolved[0]) {
            throw new RuntimeException("'" + keyPath + "' was not found in " + namespace);
        }
    ...
    private static <T> T get(Object namespace, Iterator<String> keyPathIterator, boolean[] keyResolved) {
        ...
        if (keyPathIterator.hasNext()) {
            ...
            value = (T)get(get(namespace, keyPathIterator.next(), keyResolved), keyPathIterator, keyResolved);
        } else {
            value = (T)namespace;
        }
    ...
    @SuppressWarnings("unchecked")
    private static <T> T get(Object namespace, String key, boolean[] keyResolved) {
        if (key == null) {
            throw new NullPointerException();
        }
        ...
        if (namespace instanceof List<?>) {
            ...
            if (namespace instanceof Map<?, ?>) {
                if (keyResolved != null && ((Map<String, Object>)namespace).containsKey(key)) {
                    keyResolved[0] = true;
                }

                value = ((Map<String, Object>)namespace).get(key);
            } else {
                BeanAdapter adapter = new BeanAdapter(namespace);
                ObservableValue property = adapter.getPropertyModel(key);
                if (property != null) {
    ...
    public static void set(Object namespace, String key, Object value) {
        ...
        if (namespace instanceof List<?>) {
            ...
            if (namespace instanceof Map<?, ?>) {
                ((Map<String, Object>)namespace).put(key, value);
            } else {
                BeanAdapter adapter = new BeanAdapter(namespace);
                ObservableValue property = adapter.getPropertyModel(key);
    ...
    public static boolean isDefined(Object namespace, String key) {
        ...
        if (namespace instanceof List<?>) {
            ...
            if (namespace instanceof Map<?, ?>) {
                defined = ((Map<String, Object>)namespace).containsKey(key);
            } else {
                defined = new BeanAdapter(namespace).containsKey(key);
            }
        } else {
            defined = false;
        }
