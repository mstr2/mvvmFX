...
import de.saxsys.mvvmfx.internal.viewloader.fxml.expression.Expression;
import de.saxsys.mvvmfx.internal.viewloader.fxml.expression.ExpressionValue;
import de.saxsys.mvvmfx.internal.viewloader.fxml.expression.KeyPath;
import de.saxsys.mvvmfx.internal.viewloader.fxml.expression.WritableExpressionValue;
 ...
 * @since JavaFX 2.0
 */
public class FXMLLoader implements FXMLLoaderImpl {
    ...
    private abstract class Element {
            ...
            return isTyped() ?
                new Map<String, Object>() {
                    ...
                    public boolean containsKey(Object key) {
                        return getValueAdapter().containsKey(key);
                    }
                    ...
                    public Object get(Object key) {
                        return getValueAdapter().get(key);
                    }

                    @Override
                    public Object put(String key, Object value) {
                        return getValueAdapter().put(key, value);
                    }
                    ...
                    public void putAll(Map<? extends String, ?> m) {
                        for (Map.Entry<? extends String, ?> entry : m.entrySet()) {
                            getValueAdapter().put(entry.getKey(), entry.getValue());
                        }
                    }
        ...
        public void processPropertyAttribute(Attribute attribute) throws IOException {
            ...
            boolean isAssigment = isAssigmentExpression(value);
            boolean isBinding = false, isBidiBinding = false;

            if (!isAssigment) {
                isBinding = isBindingExpression(value);
                if (!isBinding) {
                    isBidiBinding = isBidirectionalBindingExpression(value);
                }
            }

            if (isAssigment || isBinding || isBidiBinding) {
                ...
                } else if (isBidiBinding) {
                    value = value.substring(BI_DIRECTIONAL_BINDING_PREFIX.length(), value.length() - 1);
                } else {
                    value = value.substring(ASSIGNMENT_EXPRESSION_PREFIX.length(), value.length() - 1);
                }
                ...
                if (isAssigment) {
                    if (propertyModel instanceof WritableValue<?>) {
                        ((Property<Object>)propertyModel).setValue(
                            BeanAdapter.coerce(Expression.valueOf(value).evaluate(namespace, true), type));
                    } else {
                        targetAdapter.put(
                            attribute.name,
                            BeanAdapter.coerce(Expression.valueOf(value).evaluate(namespace, true), type));
                    }
                } else if (propertyModel instanceof Property<?>) {
                    ...
                    if (isBinding) {
                        ...
                        Property proxy = new WritableExpressionValue(namespace, expression, type);
                        ((Property<Object>)propertyModel).bindBidirectional(proxy);
                        bidirectionalBindingStore.add(propertyModel, proxy);
                    }
                }
            } else {
        ...
        private boolean isBidirectionalBindingExpression(String aValue) {
            return aValue.startsWith(BI_DIRECTIONAL_BINDING_PREFIX)
                    && aValue.endsWith(BI_DIRECTIONAL_BINDING_SUFFIX);
        }

        private boolean isAssigmentExpression(String aValue) {
            return aValue.startsWith(ASSIGNMENT_EXPRESSION_PREFIX)
                    && aValue.endsWith(ASSIGNMENT_EXPRESSION_SUFFIX);
        }
        ...
        private Object resolvePrefixedValue(String aValue) throws LoadException {
            if (aValue.startsWith(ESCAPE_PREFIX)) {
                ...
                return Expression.get(namespace, KeyPath.parse(aValue), true);
            }
            return aValue;
        }
        ...
        private Object getExpressionObject(String handlerValue) throws LoadException{
            if (handlerValue.startsWith(EXPRESSION_PREFIX)) {
                ...
                Object expression = Expression.get(namespace, KeyPath.parse(handlerValue), false);
                if (expression == null) {
    ...
    private abstract class ValueElement extends Element {
        ...
        private Object getListValue(Element parent, String listPropertyName, Object value) throws LoadException {
    ...
    private class ReferenceElement extends ValueElement {
        ...
        public Object constructValue() throws LoadException {
            ...
            return Expression.get(namespace, path, true);
        }
    }
    ...
    private class CopyElement extends ValueElement {
        ...
        public Object constructValue() throws LoadException {
            ...
            Object sourceValue = Expression.get(namespace, path, true);
    ...
    private final LinkedList<FXMLLoaderImpl> loaders;

    private ClassLoader classLoader = null;
    private boolean staticLoad = false;
    private LoadListener loadListener = null;

    private FXMLLoaderImpl parentLoader;
    ...
    public static final String ASSIGNMENT_EXPRESSION_PREFIX = "+{";
    public static final String ASSIGNMENT_EXPRESSION_SUFFIX = "}";
     ...
     * @since JavaFX 8.0
     */
    public static final String FX_NAMESPACE_VERSION = "2019.1";
    ...
    public FXMLLoader(URL location, ResourceBundle resources, BuilderFactory builderFactory,
            Callback<Class<?>, Object> controllerFactory, Charset charset,
            LinkedList<FXMLLoaderImpl> loaders) {
    ...
    public FXMLLoader(InputStream inputStream, URL location, ResourceBundle resources, BuilderFactory builderFactory,
            Callback<Class<?>, Object> controllerFactory, Charset charset,
            LinkedList<FXMLLoaderImpl> loaders) {
        setLocation(location);
        setResources(resources);
    ...
    @Override
    public FXMLLoaderImpl getParentLoader() {
        return parentLoader;
    }

    @Override
    public void setParentLoader(FXMLLoaderImpl loader) {
    ...
    private boolean isCyclic(
        ...
        return isCyclic(currentLoader.getParentLoader(), node);
    }
    ...
    @Override
    public <T> T load(Class<?> callerClass) throws IOException {
        return loadImpl(callerClass);
    }
    ...
    private <T> T loadImpl(final Class<?> callerClass) throws IOException {
        ...
        T value;
        try {
            inputStream = this.inputStream != null ? this.inputStream : location.openStream();
    ...
    private String constructFXMLTrace() {
        ...
        for (FXMLLoaderImpl loader : loaders) {
            messageBuilder.append(loader.getLocation() != null ? loader.getLocation().getPath() : "unknown path");

            if (loader.getCurrentElement() != null) {
    ...
    ParseTraceElement[] getParseTrace() {
        ...
        int i = 0;
        for (FXMLLoaderImpl loader : loaders) {
            parseTrace[i++] = new ParseTraceElement(loader.getLocation(), (loader.getCurrentElement() != null) ?
    ...
    private final BidirectionalBindingStore bidirectionalBindingStore = new BidirectionalBindingStore();

    private class BidirectionalBindingStore {
        private final Map<Object, List<Property>> propertyLists = new WeakHashMap<Object, List<Property>>();

        synchronized void add(Object reference, Property property) {
            List<Property> properties = propertyLists.get(reference);
            if (properties != null) {
                properties.add(property);
            } else {
                properties = new ArrayList<Property>(1);
                propertyLists.put(reference, properties);
            }
        }
    }
}