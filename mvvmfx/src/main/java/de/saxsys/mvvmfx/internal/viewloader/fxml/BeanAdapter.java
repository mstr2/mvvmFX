...
public class BeanAdapter extends AbstractMap<String, Object> {
    ...
    private Method getGetterMethod(String key) {
        Method getterMethod = localCache.getMethod(key);

        if (getterMethod == null) {
            getterMethod = localCache.getMethod(getMethodName(GET_PREFIX, key));
        }
        ...
        if (getterMethod == null) {
            throw new RuntimeException(
                "'" + key + "' can not be resolved to a public getter method of " + bean.getClass().getName());
        }
    ...
    public Object get(Object key) {
        ...
        return get(key.toString(), true);
    }

    private Object get(String key, boolean throwIfMethodNotFound) {
        ...
        } else if (throwIfMethodNotFound) {
            throw new RuntimeException(
                "'" + key + "' can not be resolved to a public method on " + bean.getClass().getName());
        }
    ...
    public <T> ObservableValue<T> getPropertyModel(String key) {
        ...
        return (ObservableValue<T>)get(key + BeanAdapter.PROPERTY_SUFFIX, false);
    }
